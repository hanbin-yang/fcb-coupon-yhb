...
/**
 * @author 唐陆军
 * @Description 每天领券限制校验前置处理
 * @createTime 2021年08月24日 15:46:00
 */
@Component
public class DayLimitBeforeSendProcessor implements BeforeSendProcessor {
    ...
    public void process(List<CouponSendContext> sendCouponContexts, CouponThemeEntity couponTheme) {
        ...
        for (CouponSendContext sendContext : sendCouponContexts) {
            ...
            if (statisticEntity.getMonthCount() >= couponTheme.getEveryMonthLimit()) {
                sendContext.error(false, "超过每个ID每月可以领取限制");
                continue;
            }
            if (statisticEntity.getTodayCount() >= couponTheme.getEveryDayLimit()) {
                sendContext.error(false, "超过每个ID每日可以领取限制");
                continue;
            }
        }
    }
