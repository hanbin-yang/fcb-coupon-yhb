...
import com.fcb.coupon.backend.model.bo.CouponThemeAddOrgBo;
...
/**
 * @author HanBin_Yang
 * @since 2021/6/21 8:52
 */
@Data
@ApiModel(description = "添加组织 入参")
public class MktUseRuleAddOrgRequest extends AbstractBaseConvertor<CouponThemeAddOrgBo> implements Serializable {
    private static final long serialVersionUID = -1518952740698643295L;
    ...
    @ApiModelProperty(value = "添加组织的集合", required = true)
    @NotEmpty(message = "merchantAddList不能没有数据")
    @JsonProperty(value = "merchantAddList")
    private List<AddOrgAo> orgAddList;

    @Override
    public CouponThemeAddOrgBo convert() {
        CouponThemeAddOrgBo bo = new CouponThemeAddOrgBo();
